...
bool halbb_la_mac_cfg_buf(struct bb_info *bb, enum la_buff_mode_t mode)
{
	...
	struct la_string_info *buf = &la->la_string_i;
	u32 addr_start = 0, addr_end = 0, buf_size_tmp = 0;
	...
	#ifdef HALBB_COMPILE_BE_SERIES
	if (bb->bb_80211spec == BB_BE_IC && mode == LA_BUFF_DEFAULT) {
		status = rtw_hal_mac_lamode_query_buf (bb->hal_com, &buf_sel);
		BB_TRACE("[%s] buf_sel : %d\n", __func__, buf_sel);
		if (status != 0) { // 0 : MACSUCCESS
			BB_WARNING("[%s] LA query buf fail! status : %d\n", __func__, status);
			return false;
		}
